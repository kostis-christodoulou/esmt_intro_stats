...
Try this for yourself. Map the column `day` onto the x axis and the column `mean_temp` onto the y axis, and use `geom_line()` to display the data.


```{r ggplot_geom_line}
ggplot(bike_weather, aes(x = day, y = mean_temp)) +
  geom_line()
```
...
## Ploting distributions of `mean_temp`
...
For boxplots, we frequently want categorical data on the x or y axis. For example, we might want a separate boxplot for each month. Try this out. Put `month` on the x axis, `mean_temp` on the y axis, and use `geom_boxplot()`.
